 ...
 * AI 助手统一配置（前端版本）
 *
 * 这个文件与后端 packages/backend/prisma/assistants.config.ts 保持同步
 *
 * 用途：
 * 1. 前端 IndexedDB 默认数据初始化
 * 2. 前端代码中的助手 ID 引用
 *
 * 重要：修改此配置时，需要同步更新后端配置文件！
 */
 ...
 * 所有内置AI助手的配置
 *
 * 注意：这些配置与后端保持完全一致，包括 ID、名称、描述等
 */
export const BUILT_IN_ASSISTANTS: LocalAssistantConfig[] = [
  // ========== 通用助手 ==========
  {
    id: "general_public",
    name: "通用助手",
